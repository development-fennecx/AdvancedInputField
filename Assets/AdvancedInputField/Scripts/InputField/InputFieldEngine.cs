...
namespace AdvancedInputFieldPlugin
{
	public class InputFieldEngine
	{
		...
		internal void InitializeRichTextEditing(string richText)
		{
			...
			{
				richTextProcessor = new RichTextProcessor(InputField.RichTextConfig.GetSupportedTags(), InputField.EmojisAllowed);
			}
			else
			{
				richTextProcessor = new RichTextProcessor(new RichTextTagInfo[0], InputField.EmojisAllowed);
			}
		...
		internal void SetSelection(int selectionStartPosition, int selectionEndPosition, bool caretIsStart = false)
		{
		...
		internal void SetRichText(string richText)
		{
			if(this.richText != richText)
			{
				...
				InputField.UpdateTextProperty(this.richText);
			}
		}
